...
if menu == "1. Regression: Predicting Attraction Ratings":
    ...
    ccc_df = pd.read_excel(
        r"D:\Tourism Experience Analytics\DATA SET\CCC.xlsx",
        sheet_name="Final_Hierarchy"
    )

    ccc_df.columns = ccc_df.columns.str.strip()

    ccc_df = ccc_df.dropna(
        subset=["Continent", "Country", "City"]
    )
